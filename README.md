...
**Docker Stage 2 — Secrets foundation is in progress.** Two production Compose-secret pilots are complete. Grafana SMTP on `ids-01` uses a host-backed secret, authenticated successfully and delivered a test notification; 303 retired plaintext Compose copies were removed. Cloudflare DDNS on TestServer now uses its native `CLOUDFLARE_API_TOKEN_FILE` interface, passed a live DNS check and is merged into `docker-env/main`. The K3s extension has completed automated inventory, central Prometheus ingestion and Grafana compliance alerting. GitOps reconciliation and preventive admission policy remain future Kubernetes work. Automated production deployment remains disabled until secrets recovery, validation and later rollout gates are satisfied.
