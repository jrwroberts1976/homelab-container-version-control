...
printf '%s\n' \
    $'host\tcontainer\tcompose_project\tcompose_service\tcompose_files\tbuild_context\tdockerfile\tdockerfile_sha256\tbuild_arg_names\tsource_git_root\tsource_git_commit\tsource_dirty\trunning_image_id\timage_created\timage_revision_label\timage_source_label\tassessment'

mapfile -t containers < <(docker ps -q --no-trunc | sort)
